...
def calculate_activation_and_std(layer:str, layer_num:int, plot:bool=False):
    ...
    if plot and layer_num % 4 == 0:
        with savefig_lock:
            ...
            # main_ax.scatter(x_toxic_answered, y_toxic_answered, alpha=0.3, s=10, label="toxic_answered", color="red")
            # main_ax.scatter(x_toxic_unanswered, y_toxic_unanswered, alpha=0.3, s=10, label="toxic_unanswered", color="orange")
            # main_ax.scatter(x_benign_answered, y_benign_answered, alpha=0.3, s=10, label="benign_answered", color="blue")
            # main_ax.scatter(x_benign_unanswered, y_benign_unanswered, alpha=0.3, s=10, label="benign_unanswered", color="green")
...
if __name__ == "__main__":
    ...
    for iteration in range(begin_iter, config.max_iter):
        ...
        # orr_results.append(1-sum(benign_val_answer_bools) / len(benign_val_answer_bools))
        # logger.info(f"benign_val_answer_bools: {benign_val_answer_bools}")
        # logger.info(f"average over refusal rate: {orr_results[-1]}")
        # asr_results.append(sum(toxic_val_answer_bools) / len(toxic_val_answer_bools))
        # logger.info(f"toxic_val_answer_bools: {toxic_val_answer_bools}")
        # logger.info(f"average toxic answer rate: {asr_results[-1]}")
        seval_attack_results.append(sum(toxic_val_answer_bools["seval_attack"]) / len(toxic_val_answer_bools["seval_attack"]))
        logger.info(f"seval_attack_results: {seval_attack_results[-1]}")
        seval_risk_results.append(sum(toxic_val_answer_bools["seval_risk"]) / len(toxic_val_answer_bools["seval_risk"]))
        logger.info(f"seval_risk_results: {seval_risk_results[-1]}")
        ...
        plt.figure(figsize=(8, 4))
        # plt.plot(range(len(asr_results)), asr_results, marker='o', label='ASR (Toxic)')
        # plt.plot(range(len(orr_results)), orr_results, marker='o', label='ORR (Benign)')
