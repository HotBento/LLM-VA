...
class DIALOGUE_Eval():
    def __init__(self, model, tokenizer, number_of_tests = None, number_of_few_shots = 0, eval_split = 'validation'):
        ...
        self.few_shots, self.eval_dataset = load_data_split(os.path.join(CURRENT_DIR, 'dataset/dialogue.pkl'), number_of_few_shots, number_of_tests)
    ...
    def evaluate(self, gen_len = 3, print_logs = False, batch_size=16, **kwargs):
        ...
        for s in range(0, len(self.eval_dataset), batch_size):
            ...
            for example in batch_examples:
                ...
                batch_prefix_lengths.append(prefix_tok_len)
            ...
            for i, (example_outputs, input_prompt, label, prefix_tok_len) in enumerate(
                zip(outputs, batch_prompts, batch_labels, batch_prefix_lengths)
            ):
                # Get the full generated text
                generated_text = self.tokenizer.decode(example_outputs, skip_special_tokens=True)
                ...
                # Calculate suffix probabilities for each option
                probs = [0 for _ in suffixes.keys()]
                gen_texts = [0 for _ in suffixes.keys()]
                
                batch_suffix_prompts = [f"{input_prompt} {suffixes[j][0]}" for j in range(len(suffixes.keys()))]
                batch_suffix_inputs = self.tokenizer(batch_suffix_prompts, return_tensors="pt", padding=True).to('cuda')
                
                with torch.no_grad():
                    batch_logits = self.model(**batch_suffix_inputs, **kwargs).logits
                if torch.cuda.memory_reserved() / (1024 * 1024) > 35000:
                    gc.collect()
                    torch.cuda.empty_cache()
                
                for j in range(len(suffixes.keys())):
                    cur_logits = batch_logits[j]
                    if 'llama' in self.model.config._name_or_path.lower():
                    ...
                    for k in range(cur_len):
                        ...
                        probs[j] += -torch.nn.functional.log_softmax(
                            cur_logits[-cur_len + k - 1, :], dim=0
                        )[cur_tok].item()
                    probs[j] /= cur_len
                    
                    gen_texts[j] = self.tokenizer.decode(
                        cur_logits[prefix_tok_len - 1 : prefix_tok_len + cur_len - 1, :].argmax(dim=-1)
                    )
                ...
                exp_temp_dict = {
                    'article': batch_articles[i],
                    'options': batch_options[i],  
                    'input_prompt': input_prompt_text,
                    'true_answer': batch_examples[i]['answers'],
                ...
                if print_logs and len(labels) % batch_size == 0:
